...
//             <div className="flex py-2 items-center justify-between text-base px-4 md:px-10 bg-[#FFFFFF] shadow-md">
...
//             <div className="hidden lg:flex py-4 px-4  bg-[#EBEBEB] text-[#111111] w-full items-center justify-between gap-5">
...
const Header = () => {
    ...
    const productNavItems = [
        { name: 'Singing Bowls', path: '/products/singing-bowls' },
        { name: 'Himalayas Ritual Items', path: '/products/ritual-items' },
        { name: 'Statues', path: '/products/statues' },
        { name: 'Himalayas Art and Paintings', path: '/products/himalayas-art&paintings' },
        { name: 'Himalayas Hemp', path: '/products/himalayas-hemp' },
        { name: 'Himalayas Crystals', path: '/products/himalayas-crystals' },
        { name: 'Healing Collections', path: '/products/himalayas-healingcollections' }
    ];
    ...
    const toggleSubMenu = (name) => {
        if (openSubMenu === name) setOpenSubMenu(null);
    ...
    return (
        <div className="flex flex-col w-full fixed top-0 z-50">
            ...
            <div className="flex py-2 items-center justify-between text-base px-4 bg-[#FFFFFF] shadow-md">
                ...
                <div className="flex gap-12 items-start text-sm border-b border-l border-r border-gray-300 rounded-b-lg p-4 bg-[#F7F7F7]">
            ...
            <div className={`fixed top-0 right-0 w-2/3 h-screen bg-[#0B4D81] text-white p-5 z-50 transition-transform ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className='flex justify-between items-center'>
                    <h1 className='text-lg font-semibold'>Menu</h1>
                    <button className="text-2xl" onClick={() => setMenuOpen(false)}><FiX /></button>
                </div>

                <ul className="flex flex-col gap-4 mt-6">
                    {[...productNavItems, ...mainNavItems].map(item => (
                        <li key={item.name}>
                            <div className="flex justify-between items-center" onClick={() => toggleSubMenu(item.name)}>
                                <NavLink to={item.path} onClick={() => setMenuOpen(false)} className="text-white py-2 block text-sm">
                                    {item.name}
                                </NavLink>
                            ...
                            {item.subItems && openSubMenu === item.name && (
                                <ul className="pl-4 mt-1 flex flex-col gap-2">
                                    {item.subItems.map(sub => (
                                        <li key={sub.name}>
                                            <NavLink to={sub.path} onClick={() => setMenuOpen(false)} className="text-white text-sm py-1 block hover:text-[#00ADA4]">
                                                {sub.name}
                                            </NavLink>
                                        </li>
                                    ))}
