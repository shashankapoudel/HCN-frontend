        ...
        <div className="flex items-center justify-center p-2 lg:p-4 mt-4 lg:mt-10 w-full md:w-1/4">
            <ul className="flex flex-col justify-center items-center md:items-end gap-10 w-full ">
                {SideBar.map((bar, index) => (
                    <div key={index} className={`relative  ${step >= index + 1 ? "text-[#0B4D81]" : "text-[#667085]"}`}>

                        {index !== SideBar.length - 1 && (
                            <span className={`absolute right-1/2 md:right-0 top-full h-10 border-l-2  ${step > index + 1 ? "border-[#0B4D81]" : "border-[#606060]"} justify-center items-center md:items-end `}></span>
                        )}

                        <li
                            className={`py-2 flex flex-col lg:flex-row items-center gap-2 lg:gap-4 ${step >= index + 1 ? "text-[#0B4D81]" : "text-gray-500"
                                }`}
                        >
