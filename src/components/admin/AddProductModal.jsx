...
const AddProductModal = ({
  ...
  const productCategory = [
    ...
    "himalayas-hemp",
    "himalayas-crystals",
    "himalayas-healingcollections",
  ];

  const categoryofSubcategory = [
    "select category",
    "antique",
    "handmade",
    "machinemade",
  ...
  useEffect(() => {
    if (subcategory) {
      setFilteredSubcategorycategory(
        subcategoriescategories[subcategory] || [],
      );
    }
  }, [subcategory]);
  ...
  const handleSubmit = async (event) => {
    ...
    formData.append(
      "note",
      noteCategory !== "Select Notes" ? noteCategory : "",
    );
    formData.append(
      "accessoriesBundle",
      accessoriesCategory !== "Select Accessories" ? accessoriesCategory : "",
    );
    ...
    try {
      const res = await fetch(
        !existingData
          ? `${BASE_URL}/product/admin/addproduct`
          : `${BASE_URL}/product/updateproduct/${existingData._id}`,
        {
          method: "POST",
          body: formData,
        },
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 text-sm">
      <div className="bg-white p-6 rounded-lg w-[85%] h-[80vh] shadow-lg flex flex-col">
        ...
        <form className="flex flex-grow w-full overflow-hidden">
          ...
          <div className="flex flex-col w-2/3 pr-4 overflow-y-auto h-full">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block mb-2">Stock(in units)*</label>
                <input
                  value={stock}
              ...
              <div>
                <label className="block mb-2">GroupId</label>
