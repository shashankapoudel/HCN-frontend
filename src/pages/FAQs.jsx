...
export default function FAQ() {
  ...
  const handleCategory = (item) => {
    setCategory(item);
    setActive(item);
    setOpenIndex(null); // Close open answer when switching category
  };
  ...
  return (
    <div className="px-4 lg:px-8 text-gray-600 w-full min-h-screen tracking-wide leading-relaxed">
      <div className="w-full flex flex-col-reverse lg:flex-row gap-3 lg:gap-8 px-2 lg:px-4">
        <div className="flex flex-col w-full lg:w-1/2 p-0 lg:p-4 gap-2 mt-4 lg:mt-16">
          <div className="flex flex-col items-start gap-2 text-sm tracking-wide w-full">
            <h2 className="text-base font-bold mb-2 text-[#0B4D81] w-full">
              Frequently Asked Questions (FAQs)
            </h2>
            <p className="mb-4 text-sm text-[#606060] leading-loose tracking-wide">
              Have Questions? Here you'll find answers to the most commonly
              asked and valued questions by our partners. We’ve put together
              helpful information to guide you through our services, processes,
              and policies—so you can find what you need quickly and easily.
            </p>
          </div>
          ...
          <div className="w-full flex flex-col gap-8">
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="flex flex-col gap-2 w-full lg:w-1/3">
                {["General", "Products", "Product Maintenance"].map(
                  (cat, index) => (
                    <button
                      key={index}
                      onClick={() => handleCategory(cat)}
              ...
              <div className="flex flex-col gap-2 w-full lg:w-1/3 items-center justify-center">
                <button
                  onClick={() => handleCategory("Customer Support")}
                  className={`flex items-center justify-between px-2 border py-8 text-sm text-[#606060] rounded-md w-full ${active === "Customer Support" ? "bg-[#0B4D81] text-white" : "bg-[#F1F1F1]"}`}
                >
              ...
              <div className="flex flex-col gap-2 w-full lg:w-1/3">
                {["Payment", "Shipping and Orders", "Refund Policy"].map(
                  (cat, index) => (
                    <button
                      key={index}
                      onClick={() => handleCategory(cat)}
              ...
              {category && (
                <div className="w-full flex flex-col gap-2">
                  {filteredFaqs.map((faq, index) => (
                    <div
                      key={index}
                      className="bg-[#F9F9F9] p-4 text-sm rounded shadow-sm"
                    >
                      <button
                        className="flex justify-between text-sm lg:text-base w-full font-medium text-left"
                        onClick={() =>
                      ...
                      {openIndex === index && faq.answer?.trim() && (
                        <div
                          className="text-[#606060] mt-4 font-light p-2"
